...
class Plane:
    ...
    def mock_values(self, scheduleTime : int = None) -> int:
        ...
        self._scheduled_time = random.randint(0, 24 * 60 * 60 - 1) if scheduleTime is None else scheduleTime
        
        self._system_time = int(random.gauss(self._scheduled_time, 300))  
        self._system_time = max(0, min(self._system_time, 24*60*60 - 1))
        
        self._fuel_seconds = random.uniform(1200, 3600) 
